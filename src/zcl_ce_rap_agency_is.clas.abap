CLASS zcl_ce_rap_agency_is DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    TYPES t_agency_range TYPE RANGE OF zrap_isz_travel_agency_es5-agencyid. "to provide filter conditions for Agenciess in form of SELECT-OPTIONS to the method get_agencies()
    TYPES t_business_data TYPE TABLE OF zrap_isz_travel_agency_es5.  "to retrieve the business data returned by our remote OData service.
...
CLASS zcl_ce_rap_agency_is IMPLEMENTATION.
  ...
  METHOD get_agencies.
    ...
    DATA(http_destination) = cl_http_destination_provider=>create_by_url( i_url = 'https://sapes5.sapdevcenter.com' ). "allows to create a http destination object based on the target URL
    http_client = cl_web_http_client_manager=>create_by_http_destination( i_destination = http_destination ).

    service_consumption_name = to_upper( 'ZSC_RAP_AGENCY_IS' ).
  ...
  METHOD if_rap_query_provider~select.
    " to retrieve the details of the incoming OData call using the object io_request

    DATA business_data TYPE t_business_data.
    DATA(top)     = io_request->get_paging( )->get_page_size( ). "to find out whether client side paging was requested with the incoming OData call
    DATA(skip)    = io_request->get_paging( )->get_offset( ).
    DATA(requested_fields)  = io_request->get_requested_elements( ).
    DATA(sort_order)    = io_request->get_sort_elements( ).
    DATA count TYPE int8.
    TRY.
        DATA(filter_condition) = io_request->get_filter( )->get_as_ranges( ). " to retrieve the filter as ranges that was used by the incoming OData request
        ...
        IF io_request->is_total_numb_of_rec_requested(  )." to find out whether the request contains a $count.
          io_response->set_total_number_of_records( count ). " to add the retrieved data
        ENDIF.
        IF io_request->is_data_requested(  ). " to find out whether the incoming request requests that business data is returned
          io_response->set_data( business_data )." to return the number of of all entries of an entity
        ENDIF.
